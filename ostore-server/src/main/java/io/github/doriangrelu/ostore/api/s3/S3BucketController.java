...
import io.github.doriangrelu.ostore.api.s3.S3Documents.BucketEntry;
import io.github.doriangrelu.ostore.api.s3.S3Documents.ListAllMyBucketsResult;
...
class S3BucketController {
    ...
    private final S3Xml xml;

    S3BucketController(BucketUseCases buckets, S3Xml xml) {
    ...
    ResponseEntity<String> listBuckets() {
        var entries = buckets.list().stream()
                ...
                .toList();
        return xml.response(HttpStatus.OK, new ListAllMyBucketsResult(S3Documents.DEFAULT_OWNER, entries));
    }
