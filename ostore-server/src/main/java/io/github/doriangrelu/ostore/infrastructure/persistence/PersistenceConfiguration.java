 ...
 */
package io.github.doriangrelu.ostore.infrastructure.persistence;
...
class PersistenceConfiguration extends AbstractJdbcConfiguration {
    ...
    /** Écriture d'un UUID dans une colonne {@code RAW(16)} (Oracle). */
    @WritingConverter
    enum UuidToBytesConverter implements Converter<UUID, byte[]> {
        INSTANCE;
    ...
    /** Lecture d'une colonne {@code RAW(16)} en UUID (Oracle). */
    @ReadingConverter
    enum BytesToUuidConverter implements Converter<byte[], UUID> {
        INSTANCE;
